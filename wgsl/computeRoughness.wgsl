...
fn computeRoughness(@builtin(global_invocation_id) id: vec3<u32>) {
    let threshold = simSettings.roughness_threshold;
    let cell = id.xy;
    ...
    if (cell.x == 0 || cell.y == 0 || cell.x >= gridSize.x - 1 || cell.y >= gridSize.y - 1) {
        ...
        var idx = 0u;
        var r: array<vec3f, 9>;
        var rSum = vec3f(0.0, 0.0, 0.0);
        
        for (var y = -1; y <= 1; y = y + 1) {
            for (var x = -1; x <= 1; x = x + 1) {
                let normal = textureLoad(normalsTexture, vec2<i32>(cell) + vec2(x, y), 0);
        ...
        let rMagnitude = length(rSum);
        roughness = 1 - rMagnitude / 9.0;
    }
    let forest = textureLoad(forestTexture, cell, 0).r;
    textureStore(roughnessTexture, cell, vec4f(roughness, f32(forest), 0.0, 0.0));
}
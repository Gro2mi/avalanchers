...
@group(0) @binding(16) var<storage, read_write> grid_mass_atomic: array<f32>;
@group(0) @binding(17) var<storage, read> grid_forces: array<vec2f>;
    ...
    if use_particle_interaction {
        ...
        for (var i: u32 = 0; i < 3; i++) {
            for (var j: u32 = 0; j < 3; j++) {
                ...
                interpolated_h += weight * grid_mass_atomic[node_idx] / (sim_settings.snow_density * sim_settings.cell_size * sim_settings.cell_size) * normal.z;
            }
        }
...
fn acceleration_by_normal_friction(effective_acceleration_normal: vec3f, mass: f32, velocity: vec3f, h: f32) -> f32 {
...
fn acceleration_by_drag_friction(effective_acceleration_normal: vec3f, mass: f32, velocity: vec3f, h: f32) -> f32 {
    let model = sim_settings.friction_model;

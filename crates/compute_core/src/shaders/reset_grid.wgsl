...
fn reset_grid(@builtin(global_invocation_id) global_id: vec3<u32>) {
    if global_id.x < sim_settings.grid_shape.x && global_id.y < sim_settings.grid_shape.y {
        ...
        grid_momentum_atomic[xy_to_idx(global_id.x, global_id.y) * 2u] = 0; // Reset grid momentum (u) for the new timestep
        grid_momentum_atomic[xy_to_idx(global_id.x, global_id.y) * 2u + 1u] = 0; // Reset grid momentum (v) for the new timestep
    }

// One pass of the jump flooding algorithm (Rong & Tan, "Jump Flooding in GPU
// with Applications to Voronoi Diagram and Distance Transform") on both
// nearest-seed fields. The run method dispatches this shader once per step
// size, with chamfer_params.step going n/2, n/4, ..., 1 for n the next power
// of two >= the larger grid dimension. The fields are read from the snapshot
// buffers written by the previous pass, so every invocation sees a consistent
// state and the result is the exact nearest-seed Voronoi.
...
@group(0) @binding(2) var<storage, read> chamfer_nearest_roi_snapshot: array<vec2u>;
@group(0) @binding(3) var<storage, read_write> chamfer_nearest_roi: array<vec2u>;
@group(0) @binding(4) var<storage, read> chamfer_nearest_sim_snapshot: array<vec2u>;
@group(0) @binding(5) var<storage, read_write> chamfer_nearest_sim: array<vec2u>;

...
@group(0) @binding(4) var<storage, read_write> grid_mass_atomic: array<atomic<u32>>; // no_atomic_float
// atomic_float @group(0) @binding(4) var<storage, read_write> grid_mass_atomic: array<atomic<f32>>;
@group(0) @binding(5) var<storage, read_write> grid_momentum_atomic: array<atomic<i32>>; // no_atomic_float
// atomic_float @group(0) @binding(5) var<storage, read_write> grid_momentum_atomic: array<atomic<f32>>;
...
fn test_transfer(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    var u = f32(atomicLoad(&grid_momentum_atomic[idx * 2])) * INV_MOMENTUM_FACTOR / (mass + 1e-6); // no_atomic_float
    var v = f32(atomicLoad(&grid_momentum_atomic[idx * 2 + 1])) * INV_MOMENTUM_FACTOR / (mass + 1e-6); // no_atomic_float

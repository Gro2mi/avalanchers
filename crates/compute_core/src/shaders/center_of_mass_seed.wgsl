// Seeds the blob label buffer for the biggest-blob center of mass pipeline:
// every mass-holding cell gets its own cell index as label, all others are
// marked NO_BLOB.
//
// Runs as a full-grid 2D dispatch before center_of_mass_propagate. Early-outs
// unless the biggest-blob mode (center_of_mass_biggest_blob setting,
// sim_settings flag bit 4) is selected.
...
@group(0) @binding(2) var<storage, read_write> blob_labels: array<u32>;

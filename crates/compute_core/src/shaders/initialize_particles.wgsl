...
@group(0) @binding(3) var terrain_metrics: texture_2d<f32>;
...
fn initialize_particles(@builtin(global_invocation_id) cell: vec3<u32>) {
    ...
    (cell.y * 19349663u) ^
    seed);
    let J = textureLoad(terrain_metrics, vec2<i32>(cell.xy), 0).z;
    let cell_volume = snow_thickness * sim_settings.cell_size * sim_settings.cell_size * J;
